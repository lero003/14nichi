...
フルMVPを機能優先で実装中です。Slice 1「オフライン閲覧基盤」、Slice 2の「全文検索・お気に入り」、Slice 3の「7日・14日の備蓄計算と不足表示」まで完了しています。次はSlice 3の「チェックリスト保存と期限表示」を、小さな検証可能な縦スライスで進めます。記事の本格拡充と監修、TestFlight準備はフルMVPの機能完成後です。
