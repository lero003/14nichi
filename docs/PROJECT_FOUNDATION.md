...
Slice 1「オフライン閲覧基盤」、Slice 2の全文検索・お気に入りに加え、Slice 3の7日・14日備蓄計算、チェックリスト保存、期限表示まで進んでいます。ガイドでは全記事の端末内検索とお気に入り絞り込み、備蓄画面では必要量・不足量、準備済み状態、期限を確認できます。製品コンテンツの監修と不足品目の買い物リストはまだです。
...
1. 不足品目を買い物リストとして確認できる導線を追加する。
2. カテゴリ・時間帯フィルタを追加してSlice 2を閉じる。
3. 個人情報スライス前に、緊急カードの脅威モデルと端末内保護方針を確定する。
4. フルMVP機能完成後に、iPhone実機テストとTestFlight準備へ進む。
