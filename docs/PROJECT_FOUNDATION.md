...
Slice 1「オフライン閲覧基盤」、Slice 2の全文検索・お気に入りに加え、Slice 3の7日・14日備蓄計算まで進んでいます。ガイドでは全記事の端末内検索とお気に入り絞り込み、備蓄画面では家族人数・利用者入力の1日量・現在量から必要量と不足量を確認できます。製品コンテンツの監修、備蓄データの保存・期限管理はまだです。
...
- 7日・14日の計算ルール（実装済み）
- 在庫と不足の表示（実装済み。入力値は現時点では再起動で消える）
- 期限と買い物リスト
- SwiftData保存
...
1. 備蓄チェックリストの端末内保存と期限表示を追加する。
2. 不足品目を買い物リストとして確認できる導線を追加する。
