...
フルMVPを機能優先で実装中です。Slice 1「オフライン閲覧基盤」、Slice 2の「全文検索・お気に入り」、Slice 3の「備蓄計算・チェックリスト保存・期限表示」まで完了しています。次はSlice 3の「不足品目の買い物リスト」を、小さな検証可能な縦スライスで進めます。記事の本格拡充と監修、TestFlight準備はフルMVPの機能完成後です。
