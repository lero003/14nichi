...
Slice 1「オフライン閲覧基盤」、Slice 2の全文検索・お気に入りに加え、Slice 3の備蓄計算、チェックリスト保存、期限表示、買い物リストまで進んでいます。ガイドでは全記事の端末内検索とお気に入り絞り込み、備蓄では必要量・不足量・期限を確認し、不足分を買い物後の在庫へ反映できます。製品コンテンツの監修とカテゴリ・時間帯フィルタはまだです。
...
1. カテゴリ・時間帯フィルタを追加してSlice 2を閉じる。
2. 個人情報スライス前に、緊急カードの脅威モデルと端末内保護方針を確定する。
3. フルMVP機能完成後に、iPhone実機テストとTestFlight準備へ進む。
