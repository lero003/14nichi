...
| 優先 | やること | 現状（リポジトリ） |
|---|---|---|
| 最優先 | iOS・Mac を同じ商品ページにするか | **同一 Bundle ID に統一済み** `jp.hazakura.FourteenDayNote` |
| 最優先 | Mac App Sandbox | **有効**（`FourteenDayNoteMac.entitlements`） |
| 最優先 | Face ID 利用目的文 | **追加済み** `NSFaceIDUsageDescription` |
| TestFlight 前 | Team、実機、Archive、Validate、Upload | **本人操作** |
| 一般公開前 | 記事 `approved` 化 | 全記事 `draft` |
| 一般公開前 | プライバシー / サポート公開ページ | 未用意（URL は本人が用意） |
| 一般公開前 | スクリーンショット・説明文 | Connect 未入力 |

バージョン: マーケティング `0.1.0` / ビルド `2`
...
| macOS | `jp.hazakura.FourteenDayNote`（同一） |

理由: 同一アプリ体験のため。App Store Connect で iOS + macOS を1レコードにできる。**初回 Upload 後は Bundle ID 変更不可**のため、Upload 前に統一した。

### 採用しなかった案

別商品（`.mac` を別レコード）は、価格・レビュー・ページが分裂するため現時点では不採用。
...
現状は全 `draft` のため distribution は**失敗する設計**。
...
- PDF 共有/印刷
- VoiceOver / 最大 Dynamic Type / ダーク / iPad / 横向き / Mac キーボード
...
前提: Xcode 26+ / 対応 SDK（リポジトリ前提は Xcode 26.6）。最低対応 OS は iOS 18 / macOS 15 のままでよい。
...
1. Scheme `FourteenDayNoteMac`
2. Destination `Any Mac`
3. Archive → Validate → Upload

Validate 成功 ≠ Upload 成功。再 Upload 時は **Build 番号を増やす**（`CURRENT_PROJECT_VERSION`）。  
内部 TestFlight は `0.1.0` のままで可。一般公開時に `1.0.0` へ上げる運用が自然。
...
- 名前 / サブタイトル / 説明 / キーワード
- スクリーンショット（iPhone・iPad 13インチ要件・Mac 16:10）
- Copyright / 年齢制限 / カテゴリ
...
| 2 | Face ID 文 | **済** |
| 3 | Mac Sandbox | **済** |
| 4 | `xcodegen generate` + 自動テスト/ビルド | エージェント可 |
...
| 10 | 内部 TestFlight | **本人** |
| 11 | Mac Archive → Validate → Upload | **本人** |
| 12 | 記事監修・`approved` | 別フェーズ |
| 13 | サポート/プライバシー公開ページ | **本人** |
| 14 | スクショ・説明文・年齢・Review 提出 | **本人** |
