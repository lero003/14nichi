...
Slice 1「オフライン閲覧基盤」、Slice 2「探索性」、Slice 3「備蓄」の機能実装が完了しています。ガイドでは全文検索・カテゴリ・行動時期・お気に入りを組み合わせ、備蓄では必要量・不足量・期限を確認し、不足分を買い物後の在庫へ反映できます。製品コンテンツの監修とSlice 4「個人情報と出力」はまだです。
...
- 大人・子ども・高齢者の人数と7日・14日の計画期間を使う備蓄計算
- 利用者入力の1人1日量・現在量から、必要量・日数目安・不足量を表示
- 家族構成・期間・数量・準備済み状態・期限をSwiftDataへ端末内保存
- 期限切れと30日以内の期限を、品目とチェックリスト概要へ表示
- 不足品目だけを表示する買い物リストと、確認後に不足分を在庫へ反映する操作
...
未監修の数量を推奨値に見せないため、1人1日量は初期値なしとし、公的な案内や家庭の事情をもとに利用者が入力する。
...
残作業と完了条件は [`FULL_MVP_COMPLETION_BRIEF.md`](./FULL_MVP_COMPLETION_BRIEF.md) を実行仕様とする。

1. 個人情報スライス前に、緊急カードの脅威モデルと端末内保護方針を確定する。
2. 合意した最小データだけで緊急カードの入力・端末内保存を実装する。
3. 公式情報リンク集と、明示的な出力同意を伴うPDF・印刷を完成させる。
4. フルMVP品質ゲートを通した後、iPhone実機テストと内部TestFlight準備へ進む。
5. 記事本文の拡充・監修はTestFlight準備後の独立フェーズで行う。
